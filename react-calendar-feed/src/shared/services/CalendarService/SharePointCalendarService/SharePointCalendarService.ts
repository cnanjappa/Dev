  ...
  public getEvents = async (): Promise<ICalendarEvent[]> => {
    ...
    try {
      const items = await web.getList(listUrl)
        .items.select("Id,Title,Description,EventDate,EndDate,fAllDayEvent,Category,Location,BannerImageUrl")
      ...
      const events: ICalendarEvent[] = items.map((item: any) => {
        ...
        const eventUrl: string = combine( this.NavigationUrl, "&ItemId=" + item.Id);
        const eventItem: ICalendarEvent = {
          ...
          backgroundImage: item.BannerImageUrl
        };
        return eventItem;
      });
